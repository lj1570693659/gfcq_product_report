<template>
  <div class="app-container">
    <el-table
      ...
      @select="selectFun"
    >
      <el-table-column prop="CNAME" label="任务名称" sortable />
      <el-table-column prop="CTASKSTATUS" label="任务状态" sortable>
        <template slot-scope="scope">
          <span v-if="scope.row.CTASKSTATUS == 1">等待</span>
          <span v-if="scope.row.CTASKSTATUS == 2">执行</span>
          <span v-if="scope.row.CTASKSTATUS == 3">完成</span>
          <span v-if="scope.row.CTASKSTATUS == 4">终止</span>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
// import Vue from 'vue'
import { getTreeByProduct } from '@/api/product/task'
...
export default {
  ...
  created() {
    ...
    getTreeByProduct({ cid: cid }).then(response => {
      this.renderDynamic = response
      console.log('this.renderDynamic-------------------', this.renderDynamic)
