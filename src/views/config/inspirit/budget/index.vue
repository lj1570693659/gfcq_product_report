<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="激励预算(万元)" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.BudgetMin/10000 }} ~ {{ scope.row.BudgetMax/10000 }}
            <span v-if="scope.row.BudgetRange == 1">(包含{{ scope.row.BudgetMin/10000 }})</span>
            <span v-if="scope.row.BudgetRange == 2">(包含{{ scope.row.BudgetMax/10000 }})</span>
            <span v-if="scope.row.BudgetRange == 3">(包含{{ scope.row.BudgetMin/10000 }}和{{ scope.row.BudgetMax/10000 }})</span>
          </span>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="预算下限" prop="budgetMin">
          <el-input
...
import { getAll, createData, modifyData, deleteData } from '@/api/config/inspirit/budget'
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        budgetMin: [{ validator: validateScoreRequire }],
        budgetMax: [{ validator: validateScoreRequire }],
        scoreRange: [{ validator: validateScoreRangeRequire }],
        budgetRange: [{ validator: validateScoreRangeRequire }]
      },
      list: null,
      total: 0,
      dataForm: {},
