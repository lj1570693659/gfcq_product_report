<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="工时比例" align="center">
        <template slot-scope="scope">
    ...
    >
      ...
      >
        <el-form-item label="比例下限" prop="scoreMin">
          <el-input
        ...
        <el-form-item label="比例上限" prop="scoreMax">
          <el-input
        ...
        <el-form-item label="比例关系" prop="scoreRange">
          <el-select v-model="temp.scoreRange" filterable default-first-option remote placeholder="请选择比例关系">
            <el-option v-for="(item,index) in scoreRangeOptions" :key="item+index" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="工时指数" prop="scoreIndex">
          <el-input
...
import { getAll, createData, modifyData, deleteData } from '@/api/config/inspirit/hours'
  ...
  data() {
    const validateScoreRequire = (rule, value, callback) => {
      if (value === '' || value === 0 || value === undefined) {
        ...
        callback(new Error('请输入工时占比值'))
      } else {
        var numReg = /^[0-9]*$/
        var numRe = new RegExp(numReg)
        if (!numRe.test(value)) {
          this.$message({
            message: '请输入数字格式的工时占比值',
            type: 'error'
          })
          callback(new Error('请输入数字格式的工时占比值'))
        }
        callback()
      }
    }
    const validateRadioRequire = (rule, value, callback) => {
      if (value === '' || value === undefined) {
        ...
        if (!numRe.test(value)) {
          this.$message({
            message: '请输入数字格式的分配比例值',
            type: 'error'
          })
          callback(new Error('请输入数字格式的分配比例值'))
        }
        callback()
      }
    }
    ...
    return {
      ...
      rules: {
        scoreMin: [{ validator: validateRadioRequire }],
        scoreMax: [{ validator: validateRadioRequire }],
        scoreIndex: [{ validator: validateScoreRequire }],
