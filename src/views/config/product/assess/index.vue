<template>
  <div class="app-container">
    ...
    <el-table
      ...
      @select="selectFun"
    >
      <el-table-column prop="evaluateDimensions" label="评价标准" />
      <el-table-column prop="evaluateCriteria" type="hidden" label="评价标准" />
      <el-table-column prop="scoreCriteria" label="评分标准" />
    ...
    >
      ...
      >
        <el-form-item v-if="dialogStatus !== 'update'" label="层级：" prop="evaluateId">
          <el-select v-model="temp.evaluateId" placeholder="请选择层级" size="small">
            <el-option key="0" label="顶层标准" value="0" />
            <el-option v-for="item in locationData" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="评价标准：" prop="evaluateCriteria">
          <el-input
            v-model="temp.evaluateCriteria"
            size="small"
            autocomplete="off"
            placeholder="请输入标签名称"
          />
...
<script>
// import Vue from 'vue'
import { getLists, createData, modifyData, deleteData } from '@/api/config/product/access'
export default {
  name: 'App',
  ...
  methods: {
    ...
    initData(data) {
      data.forEach((item) => {
        ...
        if (item.evaluateId === 0) {
          this.locationData.push({ id: item.id, name: item.evaluateDimensions })
          this.tableShowData[item.id] = item.evaluateDimensions
        }
      })
    },
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var data = {
            id: 0,
            evaluateCriteria: this.temp.evaluateCriteria,
            weight: this.temp.weight,
            evaluateId: this.temp.evaluateId,
            scoreCriteria: this.temp.scoreCriteria,
            remark: this.temp.remark
          }
          if (Number(this.temp.evaluateId) === 0) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          var data = {
            id: this.temp.id,
            evaluateDimensions: this.temp.evaluateDimensions,
            evaluateCriteria: this.temp.evaluateCriteria,
            weight: this.temp.weight,
            evaluateId: this.temp.evaluateId,
            scoreCriteria: this.temp.scoreCriteria,
