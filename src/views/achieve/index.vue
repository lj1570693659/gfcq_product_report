<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="shouldSubmitNumber" label="应提交数量" sortable />
      <el-table-column prop="realSubmitNumber" label="实际提交数量" sortable />
      <el-table-column label="操作" :align="alignDir" width="180">
        <template slot-scope="scope">
          <el-button v-if="scope.row.type != 1" type="text" size="small" @click="handleUpdate(scope.row)">详情</el-button>
          <el-button v-if="scope.row.type != 1" type="text" size="small" @click="handleTask(scope.row)">任务</el-button>
          <el-button v-if="scope.row.type != 1" type="text" size="small" @click="handleAggre(scope.row)">交付物汇总</el-button>
          <el-button v-if="scope.row.type != 1" type="text" size="small" @click="handleDepart(scope.row)">各部门提交</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
// import Vue from 'vue'
import { getDocCate } from '@/api/product/task'
...
export default {
  ...
  created() {
    ...
    getDocCate({ cid: cid }).then(response => {
